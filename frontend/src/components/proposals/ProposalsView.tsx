...
interface NoteEditorRowProps extends SortableEditableRowProps {
  ...
  onToggleNoteInitials: (index: number) => void;
...
const defaultCreditCardNote =
  "* THERE WILL BE A 3% FEE ADDED TO ALL CREDIT CARD TRANSACTIONS *";
          ...
          <div className="flex items-center justify-between">
            <label className="inline-flex items-center gap-2 text-sm text-zinc-300">
              <input
                ...
                onChange={() => onToggleNoteInitials(index)}
              />
...
function parseQuoteMeta(descriptionBody: string): {
  notes: EditableQuoteNote[];
  includeTotals: boolean;
  paymentSchedule: string;
} {
...
export function ProposalsView({
  ...
  const [isCustomCabinetSuggestionsOpen, setIsCustomCabinetSuggestionsOpen] =
    useState(false);
  ...
  const proposalDefaults = useMemo(
    () => ({
      ...
      creditCardNote:
        companySettings?.defaultCreditCardNote || defaultCreditCardNote,
      signatureNote:
        companySettings?.defaultSignatureNote || defaultSignatureNote,
    }),
    [companySettings],
  );
  ...
  const matchingCustomCabinetJobs = useMemo(() => {
    ...
    return estimates.filter((estimate) => {
      const estimateCustomerName = estimate.customer?.name
        ...
        .toLowerCase();
      return (
        Boolean(estimateCustomerName) && estimateCustomerName === customerName
      );
    });
  ...
  const filteredCustomCabinetSuggestions = useMemo(() => {
    ...
    return matchingCustomCabinetJobs.filter(
      (estimate) =>
    ...
    try {
      ...
      if (quote) {
        ...
        if (!nextForm.paymentSchedule.trim()) {
          nextForm.paymentSchedule = getDefaultPaymentSchedule(
            proposalDefaults.paymentsNote,
          );
        }
        setForm(nextForm);
    ...
    try {
      ...
      if (quote) {
        ...
        const matchingRate = taxRates.find(
          (r) =>
            Math.abs(
              (quote.subtotal || 0) * (r.rate / 100) - (quote.tax || 0),
            ) < 0.01,
        );
        if (matchingRate) {
  ...
  const handleSaveProposalPDF = async () => {
    ...
    try {
      ...
      filePath = await GenerateProposalPDF(currentQuote.id, printHtml);
    ...
    if (companySettings?.openPdfAfterSave ?? true) {
      try {
        ...
        console.error("PDF saved but failed to open proposal PDF:", error);
  ...
  const toggleNoteInitials = (index: number) => {
    setForm((prev) => ({
      ...
      notes: prev.notes.map((note, noteIndex) =>
        noteIndex === index
          ? { ...note, includeInitials: !note.includeInitials }
          : note,
      ),
    }));
  };
  ...
  useEffect(() => {
    ...
    quickCreateForCustomer,
    loading,
    lastHandledQuickCreateToken,
    onQuickCreateHandled,
  ]);

  useEffect(() => {
    if (!openProposalRecord || loading) {
      return;
    }

    if (openProposalRecord.token === lastHandledOpenToken) {
      return;
    }

    setLastHandledOpenToken(openProposalRecord.token);
    void handleLoadQuote(openProposalRecord.id);
    onOpenProposalHandled?.();
  }, [
    openProposalRecord,
    loading,
  ...
  const handleDuplicateQuote = async (quoteId: number) => {
    try {
      const duplicated = await DuplicateManualQuote(quoteId);
      const nextQuote = (duplicated as ManualQuote) || null;
      if (nextQuote?.id) {
        await fetchQuotesPage();
        await handleLoadQuote(nextQuote.id);
                  ...
                  {quotes.map((quote) => (
                    ...
                    >
                      <td className="px-4 py-3 text-sm font-medium text-zinc-100">
                        {quote.quoteNumber ||
                          `P-${quote.id.toString().padStart(4, "0")}`}
                      </td>
                      <td className="px-4 py-3 text-sm font-medium text-zinc-100">
                        {quote.customer?.name || "-"}
                      </td>
            ...
            <div className="flex items-center justify-between border-t border-zinc-800 px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <p className="text-xs text-zinc-400">
                  Showing{" "}
                  {totalQuotes === 0 ? 0 : (currentPage - 1) * pageSize + 1}-
                  {Math.min(currentPage * pageSize, totalQuotes)} of{" "}
                  {totalQuotes}
                </p>
              </div>
        ...
        >
          <div className="space-y-4">
            <p className="text-zinc-300">
              ...
              {quoteToDelete
                ? ` "${quoteToDelete.quoteNumber || quoteToDelete.jobName}"`
                : " this quote"}
              ?
            </p>
        ...
        <div className="manual-quote-sheet max-w-[900px] mx-auto px-8 py-7 text-black bg-white">
              ...
              <div className="mt-6 space-y-2 text-[12px] leading-[1.4] font-semibold">
                {form.notes.map((note, index) => (
                  ...
                  >
                    ...
                    {note.includeInitials && (
                      <span className="inline-block w-12 border-b border-black" />
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-[1fr_420px] gap-8 mt-16 items-end">
            <div className="text-[10px] leading-[1.35] max-w-[280px]">
              ...
              {proposalDefaults.termsBlock3 && (
                <p className="mt-3">{proposalDefaults.termsBlock3}</p>
              )}
            </div>
            ...
            <div className="pt-2">
              ...
              <div className="w-full text-[14px] leading-tight">
                ...
                <div className="flex justify-between py-1.5 mt-2 font-semibold">
                  ...
                  <span>
                    {form.includeTotals
                      ? formatCurrency(form.depositAmount)
                      : ""}
                  </span>
                </div>
                <div className="flex justify-between border-t border-black py-1.5 font-semibold">
                  ...
                  <span>
                    {form.includeTotals ? formatCurrency(form.amountDue) : ""}
                  </span>
                </div>
              </div>
            </div>
          ...
          <p className="text-center text-[11px] mt-1 font-medium">
            {proposalDefaults.signatureNote}
          </p>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {isCreatingProposal && (
              <Button
                variant="ghost"
                onClick={() => void handleCancelNewProposal()}
              >
                Cancel
              </Button>
            )}
            ...
            <Button
              variant="secondary"
              onClick={() => void handleSaveProposalPDF()}
            >
                    ...
                    <div className="grid grid-cols-[1fr_140px_110px] gap-2">
                      <div className="relative">
                        <Input
                          ...
                          onFocus={() =>
                            setIsCustomCabinetSuggestionsOpen(true)
                          }
                          onBlur={() => {
                            setTimeout(
                              () => setIsCustomCabinetSuggestionsOpen(false),
                              ...
                              {filteredCustomCabinetSuggestions.map(
                                (estimate) => (
                                  <button
                                    ...
                                    onMouseDown={(event) => {
                                      event.preventDefault();
                                      applyCustomCabinetEstimateToDraft(
                                        estimate,
                                      );
                                    }}
                                  >
                      ...
                      <label className="inline-flex items-center gap-2 text-sm text-zinc-300">
                        <input
                          ...
                          onChange={(e) =>
                            setDraftNote((prev) => ({
                              ...prev,
                              includeInitials: e.target.checked,
                            }))
                          }
                        />
                        Include initials line
                  ...
                  <div className="rounded-lg border border-zinc-700 overflow-hidden">
                    {form.notes.length === 0 ? (
                      ...
                      <SortableList
                        ...
                        className="space-y-2 p-2"
                        renderItem={(note) => {
                          const index = form.notes.findIndex(
                          ...
                          return (
                            <NoteEditorRow
                              ...
                              onToggleNoteInitials={toggleNoteInitials}
